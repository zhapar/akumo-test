export function InstagramLogo(props) {
  return (
    <svg
      width="30"
      height="30"
      {...props}
      viewBox="0 0 30 30"
      fill="none"
      xmlns="http://www.w3.org/2000/svg">
      <path
        d="M21.25 2.5H8.75C5.29822 2.5 2.5 5.29822 2.5 8.75V21.25C2.5 24.7018 5.29822 27.5 8.75 27.5H21.25C24.7018 27.5 27.5 24.7018 27.5 21.25V8.75C27.5 5.29822 24.7018 2.5 21.25 2.5Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M20.0001 14.2124C20.1544 15.2527 19.9767 16.3152 19.4923 17.2487C19.0079 18.1822 18.2416 18.9392 17.3022 19.412C16.3628 19.8848 15.2982 20.0494 14.2599 19.8823C13.2215 19.7152 12.2623 19.225 11.5187 18.4813C10.775 17.7377 10.2848 16.7785 10.1177 15.7402C9.95063 14.7018 10.1152 13.6373 10.588 12.6979C11.0609 11.7585 11.8179 10.9921 12.7514 10.5077C13.6849 10.0233 14.7473 9.84562 15.7876 9.99989C16.8488 10.1572 17.8312 10.6517 18.5897 11.4103C19.3483 12.1688 19.8428 13.1512 20.0001 14.2124Z"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M21.875 8.125H21.8883"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}
